...
public class ConnectionHandler {
    ...
    static int sendMulticastPORT = 10001;                                           // <---  NOVO
    ...
    ConnectionHandler()
    {
        ...
        { 
            ...
            group = InetAddress.getByName("225.0.0.1");                               // <---  NOVO
        }
    ...
    private void createChallengeRequest(String name, String ano, String mes, String dia, String hora, String min, String seg){
        try{
            ...
            if(!ano.equals("") && !mes.equals("") && !dia.equals("") && !hora.equals("") && !min.equals("") && !seg.equals("")){
                // buffers size calculation
                int bufferSize = 8 + name.length() + ano.length() + mes.length() + dia.length() + hora.length() + min.length() + seg.length();

                sendData = new byte[bufferSize];

                fieldList = new String[7];
                fieldList[0] = name;
                fieldList[1] = ano;
                fieldList[2] = mes;
                fieldList[3] = dia;
                ...
                // buildPDU(ver, sec, label, type, numFields, fieldList)
                sendData = buildPDU(0, 0, 1, 8, 7, fieldList);
            }
            else{
                
                // buffers size calculation
                int bufferSize = 8 + name.length();
                ...
                // buildPDU(ver, sec, label, type, numFields, fieldList)
                sendData = buildPDU(0, 0, 1, 8, 1, fieldList);
    ...
    private void acceptChallengeRequest(String challengeName){
        try{
            ...
            // buffers size calculation
            int bufferSize = 8 + challengeName.length();
            ...
            // buildPDU(ver, sec, label, type, numFields, fieldList)
            sendData = buildPDU(0, 0, 1, 9, 1, fieldList);
